...
# 3. CONTENIDO PRIVADO (DASHBOARD)
    ...
    # ------------------------------------------------------------------
    # IMPORTANTE: Este 'elif' debe estar alineado con el 'if' de la página principal
        ...
        paqueterias_pendientes = sorted(df_pendientes["FLETERA"].unique())
        seleccion = st.multiselect("Filtrar por Paquetería:", options=paqueterias_pendientes)

        if seleccion:
            df_ver = df_pendientes[df_pendientes["FLETERA"].isin(seleccion)].copy()
            df_ver["DÍAS TRANS."] = (pd.Timestamp.now() - df_ver["FECHA DE ENVÍO"]).dt.days
            df_ver["DÍAS ATRASO"] = (pd.Timestamp.now() - df_ver["PROMESA DE ENTREGA"]).dt.days
            df_ver["DÍAS ATRASO"] = df_ver["DÍAS ATRASO"].apply(lambda x: x if x > 0 else 0)
            
            cols = ["NOMBRE DEL CLIENTE", "NÚMERO DE PEDIDO", "FLETERA", "FECHA DE ENVÍO", "PROMESA DE ENTREGA", "NÚMERO DE GUÍA", "DÍAS TRANS.", "DÍAS ATRASO"]
            st.dataframe(df_ver[cols], use_container_width=True, hide_index=True)
            st.divider()

        # --- TARJETAS MÉTRICAS ---
        total_p = len(df_kpi)
        pend_p = len(df_pendientes)
        ...
        estilo_m = "background-color:#11141C; padding:20px; border-radius:15px; border:1px solid #2D333F; text-align:center;"
        tit_m = "color:yellow; font-size:13px; font-weight:bold; border-bottom:1px solid #2D333F; margin-bottom:10px;"
        
        m1.markdown(f"<div style='{estilo_m}'><div style='{tit_m}'>PEDIDOS TOTALES</div><div style='font-size:24px;'>{total_p}</div></div>", unsafe_allow_html=True)
        m2.markdown(f"<div style='{estilo_m}'><div style='{tit_m}'>SIN ENTREGAR</div><div style='font-size:24px; color:#38bdf8;'>{pend_p}</div></div>", unsafe_allow_html=True)
        m3.markdown(f"<div style='{estilo_m}'><div style='{tit_m}'>EFICIENCIA</div><div style='font-size:24px; color:#00FFAA;'>{eficiencia_p:.1f}%</div></div>", unsafe_allow_html=True)
        color_c = "#FF4B4B" if costo_caja_p > 60 else "#00FFAA"
        m4.markdown(f"<div style='{estilo_m}'><div style='{tit_m}'>COSTO/CAJA</div><div style='font-size:24px; color:{color_c};'>${costo_caja_p:,.2f}</div></div>", unsafe_allow_html=True)
        ...
        # Gráfico 2: Eficiencia (Alineación corregida)
