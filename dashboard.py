...
# 3. CONTENIDO PRIVADO (DASHBOARD)
    ...
    if st.session_state.pagina == "principal":
        ...
        # TÍTULO Y ENCABEZADO
        # --------------------------------------------------
        st.markdown("<style>@keyframes floatBox{0%{transform:translateY(0);}50%{transform:translateY(-4px);}100%{transform:translateY(0);}}@keyframes textIn{0%{opacity:0;transform:translateY(6px);}100%{opacity:1;transform:translateY(0);}}@keyframes shimmerLoop{0%{background-position:-200%;}8%{background-position:200%;}100%{background-position:200%;}}</style><h2 style='text-align:center;display:flex;align-items:center;justify-content:center;gap:14px;font-weight:600;'><svg style='animation:floatBox 3.2s ease-in-out infinite;' width='34' height='34' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M3 7L12 2L21 7L12 12L3 7Z' stroke='#00FFAA' stroke-width='1.5' stroke-linejoin='round'/><path d='M3 7V17L12 22L21 17V7' stroke='#00FFAA' stroke-width='1.5' stroke-linejoin='round'/><path d='M12 12V22' stroke='#00FFAA' stroke-width='1.5'/><path d='M7.5 4.8L16.5 9.3' stroke='#00FFAA' stroke-width='1.1' opacity='0.6'/></svg><span style='opacity:0;animation:textIn 0.9s ease-out forwards, shimmerLoop 12s ease-in-out infinite;background:linear-gradient(90deg,#FFFFFF,#00FFAA,#FFFFFF);background-size:200%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;'>Control de Embarques</span></h2>", unsafe_allow_html=True)
