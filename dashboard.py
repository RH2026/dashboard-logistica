            ...
            if df_busqueda.empty:
                ...
                for index, row in df_busqueda.iterrows():
                    ...
                    with c1:
                        ...
                        st.markdown(f"<div style='{estilo_card}'><div style='color:yellow;font-weight:bold;text-align:center;'>Información Cliente</div><b>NO CLIENTE:</b> {row.get('NO CLIENTE')}<br><b>NOMBRE:</b> {row.get('NOMBRE DEL CLIENTE')}<br><b>DESTINO:</b> {row.get('DESTINO')}<br><b>FLETERA:</b> <span style='color:#38bdf8;font-weight:bold;{row.get('FLETERA')}<br><b>NÚMERO DE GUÍA:</b> <span style='color:#38bdf8;font-weight:bold;'>{row.get('NÚMERO DE GUÍA','—')}</span><br><b>COSTO:</b> <span style='color:#22c55e;'>{costo_mxn}</span></div>", unsafe_allow_html=True)
                    with c2:
